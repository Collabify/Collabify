 ...
 */
public class Json {
    public final static String TAG = Json.class.getSimpleName();
  ...
  /* PUT */
    ...
    /***********************************************************************************************/
    /**
     * Send an http DELETE to the specified uri
     * not really a json thing so not sure if it belongs here
     * @param uri URI to send DELETE to
     * @param headerKey     array of header keys to send
     * @param headerValue   array of header values to send
     * @return response from server
     */
    public static String delete(String uri, String[] headerKey, String[] headerValue){
        String result = "";
        try{
            HttpClient httpClient = new DefaultHttpClient();
            ...
            if(headerKey != null && headerValue != null && headerKey.length == headerValue.length){
                for(int i = 0; i < headerKey.length; i++) {
                    httpDelete.addHeader(headerKey[i], headerValue[i]);
                }
            }
            ...
            Log.d(TAG, "ClientProtocolException when executing httpDelete: " + e.getMessage());
            e.printStackTrace();
        } catch (IOException e) {
            Log.d(TAG, "IOException when executing httpDelete: " + e.getMessage());
            e.printStackTrace();
        }
        return result;
    }
